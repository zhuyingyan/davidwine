...
J.Transition = (function($){
    var isBack,$current,$target,transitionName,
        animationClass = {
            ...
            opacityShow: [['','opacityIn'],['opacityOut','']]
        };
...
J.Welcome = (function($){
    ...
    var showWelcome = function(){
        $.ajax({
            url : J.settings.basePagePath+'welcome2.html',
            timeout : 5000,
            async : false,
            success : function(html){
                //添加到dom树中
                $('body').append(html);
                var $welpage1=$(".slide_it1");
                setTimeout(function(){
                     J.anim(".slide_it1","bounceOut",function(){
                     $(this).hide();
                     //console.log("test");
                     });
                },3500);
                ...
                $("#welcome_btn").on("touchstart",function(){
                    J.Welcome.hide();
                });
                /*new J.Slider({
    ...
    var hideWelcome = function(){
        J.anim('#jingle_welcome','bounceOut',function(){
            $(this).remove();
...
J.Popup = (function($){
    var _popup,_mask,transition,clickMask2close,
        ...
        TEMPLATE = {
            ...
            loading : '<p><i class="icon spinner"></i>{title}</p>'
        };
    ...
    var loading = function(text){
        var markup = TEMPLATE.loading.replace('{title}',text||'Loading...');
        show({
            html : markup,
            pos : 'loading',
            opacity :.1,
...
;(function($){
    ...
    function Refresh(selector,type,callback){
        var iscroll, scroller,refreshEl,iconEl,labelEl,topOffset,isPullDown,numc = 0,
            options = {
                selector : undefined,
                type : 'pullDown',//pullDown|pullUp 默认为pullDown
                minPullHeight : 50,//拉动的像素相对值，超过才会翻转
                pullText: "下拉刷新...",
                releaseText: "松开立即刷新...",
                refreshText: "刷新中...",
                refreshTip : false,//下拉时显示的文本，比如：最后更新时间:2013-....
                onPullIcon : 'arrow_down_2',
                onReleaseIcon  : 'icon_reverse',
        ...
        var _init = function(opts){
            ...
            var refreshTpl = $(scroller).find('.pull_scroll'),refreshEl;
            
            if(!refreshTpl.length){
                refreshTpl = '<div class="pull_scroll"><span class="pull_scroll_icon '+opts.onPullIcon
                    +'"></span><span class="pull_scroll_label">'
                    +opts.pullText+'</span>'
                    +(opts.refreshTip?'<div class="refresh_tip">'+opts.refreshTip+'</div>':'')+'</div>';
                if(isPullDown){
                ...
                refreshEl = $(refreshTpl);
            }
            
            topOffset = refreshEl.height();
            iconEl = refreshEl.find('.pull_scroll_icon');
            labelEl = refreshEl.find('.pull_scroll_label');
        }
        ...
        var _excuteScroll = function(opts){
            return J.Scroll(opts.selector,{
                ...
                onScrollMove : function(){
                    if(numc===0){
                        refreshEl = $(scroller).find('.pull_scroll');
                ...
                onScrollEnd : function(){
					//console.log(iconEl.hasClass(opts.onReleaseIcon));
